...
def plot(x, y, type, filepath):
    ...
    percent = 100.*y/y.sum()
    cdict = dict(zip(x, plt.cm.tab10.colors if len(x)
                 <= 10 else plt.cm.tab20.colors))

    patches, texts = plt.pie(y, startangle=90, radius=50, colors=None if len(
        x) > 20 else [cdict[v] for v in x])
